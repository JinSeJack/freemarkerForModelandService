...
public class ${className} extends JdbcDao<${modelClassName}> implements ${daoInterfaceName} {
	...
	public long insert(${modelClassName} obj){
	   PreparedStatementCreator stat = new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
			    String sql="insert into ${modelClassName} (${indexName},<#list fields as e>${e.beanName}<#if e_has_next>,</#if></#list>) ";
		        sql=sql+" values(seq${indexName}.NEXTVAL,<#list fields as e>?<#if e_has_next>,</#if></#list>)";
				String generatedColumns[] = {"${indexName}"};
				PreparedStatement ps = con.prepareStatement(sql,generatedColumns);
				<#list fields as e>
				  <#if e.beanType=='Integer'>ps.setInteger(${e_index+1}, obj.${e.getterName}());</#if>
				  <#if e.beanType=='Long'>ps.setLong(${e_index+1}, obj.${e.getterName}());</#if>
				  <#if e.beanType=='Double'>ps.setDouble(${e_index+1}, obj.${e.getterName}());</#if>
				  <#if e.beanType=='String'>ps.setString(${e_index+1}, obj.${e.getterName}());</#if>
				  <#if e.beanType=='Date'>ps.setTimestamp(${e_index+1}, new Timestamp(obj.${e.getterName}().getTime()));</#if>
				</#list>
				return ps;
			}
		};
     ...
     */
	@Override
	public Pager<${modelClassName}> findAllPaged(long pageIndex,String sortField,String sortType){
		String sql="select * from ${modelClassName}";
